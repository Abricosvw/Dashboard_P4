...
static esp_err_t audio_mute_function(AUDIO_PLAYER_MUTE_SETTING setting) {
  // Volume saved when muting and restored when unmuting. Restoring volume is
  // necessary as es8311_set_voice_mute(true) results in voice volume (REG32)
  // being set to zero.
  ...
  if (setting == AUDIO_PLAYER_UNMUTE) {
    ESP_RETURN_ON_ERROR(
        esp_codec_dev_set_out_vol(play_dev_handle, _vloume_intensity), TAG,
...
esp_err_t bsp_extra_codec_set_fs(uint32_t rate, uint32_t bits_cfg, uint8_t ch) {
  esp_err_t ret = ESP_OK;
...
esp_err_t bsp_extra_codec_volume_set(int volume, int *volume_set) {
  ESP_RETURN_ON_ERROR(esp_codec_dev_set_out_vol(play_dev_handle, volume), TAG,
                      "Set Codec volume failed");
...
int bsp_extra_codec_volume_get(void) { return _vloume_intensity; }
...
esp_err_t bsp_extra_codec_dev_resume(void) {
  return bsp_extra_codec_set_fs(CODEC_DEFAULT_SAMPLE_RATE,
                                CODEC_DEFAULT_BIT_WIDTH, CODEC_DEFAULT_CHANNEL);
}
...
esp_err_t bsp_extra_codec_init() {
  ...
  bsp_extra_codec_set_fs(CODEC_DEFAULT_SAMPLE_RATE, CODEC_DEFAULT_BIT_WIDTH,
...
esp_err_t bsp_extra_player_init(void) {
  ...
  audio_player_config_t config = {.mute_fn = audio_mute_function,
                                  ...
                                  .priority = 5};
  ESP_RETURN_ON_ERROR(audio_player_new(config), TAG,
                      "audio_player_init failed");
...
esp_err_t
bsp_extra_file_instance_init(const char *path,
