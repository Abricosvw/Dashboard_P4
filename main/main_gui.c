...
static void touch_callback(lv_indev_drv_t *drv, lv_indev_data_t *data) {
  ...
  bool touched = esp_lcd_touch_get_coordinates(tp, x, y, NULL, &cnt, 1);
...
esp_err_t main_gui_init(esp_lcd_panel_handle_t panel_handle) {
  ...
  // 5. Initialize Touchscreen
  ESP_LOGI(TAG, "Initializing touchscreen...");
  esp_lcd_panel_io_handle_t tp_io_handle = NULL;
  esp_lcd_touch_handle_t tp_handle = NULL;
  bool touch_found = false;

  vTaskDelay(pdMS_TO_TICKS(100)); // Settle bus after DSI init

  // Auto-detect touchscreen type
  uint8_t detected_addr = 0;
  if (i2c_probe(0x5D))
    detected_addr = 0x5D;
  else if (i2c_probe(0x14))
    detected_addr = 0x14;

  if (detected_addr != 0) {
    ESP_LOGI(TAG, "Found GT911 at 0x%02X", detected_addr);
    esp_lcd_panel_io_i2c_config_t tp_io_config =
        ESP_LCD_TOUCH_IO_I2C_GT911_CONFIG();
    tp_io_config.dev_addr = detected_addr;
    tp_io_config.scl_speed_hz = 400000;

    if (esp_lcd_new_panel_io_i2c(i2c1_bus, &tp_io_config, &tp_io_handle) ==
        ESP_OK) {
      esp_lcd_touch_config_t tp_cfg = {
          .x_max = LCD_H_RES,
          .y_max = LCD_V_RES,
          .rst_gpio_num = -1, // Reset handled in main.c
          .int_gpio_num = -1,
          .flags =
              {
                  .swap_xy = 0,
      ...
      if (esp_lcd_touch_new_i2c_gt911(tp_io_handle, &tp_cfg, &tp_handle) ==
          ESP_OK) {
        touch_found = true;
      }
    }
  } else if (i2c_probe(0x38)) {
    ESP_LOGI(TAG, "Found FT5x06 at 0x38");
    esp_lcd_panel_io_i2c_config_t tp_io_config =
    ...
    if (esp_lcd_new_panel_io_i2c(i2c1_bus, &tp_io_config, &tp_io_handle) ==
      ...
      if (esp_lcd_touch_new_i2c_ft5x06(tp_io_handle, &tp_cfg, &tp_handle) ==
          ESP_OK) {
        touch_found = true;
      }
    }
  ...
  if (touch_found) {
    ...
    ESP_LOGI(TAG, "Touchscreen registered successfully");
  } else {
    ESP_LOGW(TAG,
             "Touchscreen not detected - continuing without touch support");
  }
