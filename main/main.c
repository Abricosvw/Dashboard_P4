...
void app_main(void) {
  // OPTIMIZED BOOT SEQUENCE (Total estimated time to UI: < 600ms)
  ...
  // 4. Audio Init (~20 ms)
  // Initialize I2S and Codec.
  ESP_LOGI(TAG, "4. Initializing Audio...");
  audio_init();
  // audio_play_tone(440, 100); // Optional boot beep

  // 5. Initialize SD Card (~100 ms)
  // Load settings and assets.
  ESP_LOGI(TAG, "5. Initializing SD Card...");
  ...
  // 6. Start Background Managers
  can_init();
  // ai_assistant_init();

  // 7. Initialize WiFi (Last / Background) (~1000-3000 ms)
  // Connecting to WiFi is slow, so we do it last to not block the UI startup.
  ESP_LOGI(TAG, "7. Initializing WiFi (Background)...");
